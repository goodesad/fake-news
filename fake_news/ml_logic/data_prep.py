...
import pickle
from fake_news.ml_logic.preprocessing import preprocessing
...
def prepare_data_news_notnews(X,y):
    ...
    with open ('fake_news/saved/pickles/news_notnews_tokenizer.pickle', 'wb') as handle:
...
def prepare_pred_news_notnews(text):
    ...
    text = preprocessing(text) # this tiny line of code makes all the difference in the world !!!
...
def prepare_data_real_fakenews(X,y):
    ...
    # save fitted tokenizer as pickle file
    with open ('fake_news/saved/pickles/real_fakenews_tokenizer.pickle', 'wb') as handle:
        pickle.dump(tokenizer, handle, protocol=pickle.HIGHEST_PROTOCOL)
